...
export default class Dropdown extends PureComponent {
  ...
  renderItem({item, index}) {
    ...
    let title = label ? label : value;

    // let color = index === selected ? colors.LABEL : colors.TEXTINPUT_FONT;
    let color = 'red';
    const fontFamily =
      index === selected ? FONT_FAMILY.SEMI_BOLD : FONT_FAMILY.REGULAR;
    let textStyle = {color, fontSize, fontFamily};
    props.style = [
      style,
      {
        paddingLeft: this.props.leftInset ? this.props.leftInset : leftInset,
        paddingRight: this.props.rightInset
          ? this.props.rightInset
          : rightInset,
        // backgroundColor:
        //   index === selected
        //     ? colors.DROPDOWN_SELECTED_BACK
        //     : colors.DROPDOWN_BACK,
  ...
  render() {
    ...
    let {left, top, opacity, selected, modal} = this.state;
    ...
    let height = 2 * itemPadding + itemSize * (visibleItemCount + 1);
              ...
              {hideHeader ? null : (
                <View style={[styles.selectView]}>
                  <Text style={styles.selectText}>
                    {headerTitle ? headerTitle : 'SELECT_METER'}
                  </Text>
                </View>
              )}
              <FlatList
